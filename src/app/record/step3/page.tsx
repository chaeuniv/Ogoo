...
// 선택 시 아이콘 모양(알파)을 따라 그림자가 생기도록 SVG 자체에 drop-shadow 필터 적용
...
// 소확행: 연보라 불규칙 blob
function SohwaengBlob({ selected }: { selected?: boolean }) {
  return (
    <svg viewBox="1648 432 236 240" className="w-full h-full" style={shapeFilterStyle(selected)} fill="none" xmlns="http://www.w3.org/2000/svg">
...
// 스트레스: 분홍 뾰족 별
function StressStar({ selected }: { selected?: boolean }) {
  return (
    <svg viewBox="1296 418 252 252" className="w-full h-full" style={shapeFilterStyle(selected)} fill="none" xmlns="http://www.w3.org/2000/svg">
...
// 합리적 소비: 연두 다이아몬드
function RationalDiamond({ selected }: { selected?: boolean }) {
  return (
    <svg viewBox="868 415 292 292" className="w-full h-full" style={shapeFilterStyle(selected)} fill="none" xmlns="http://www.w3.org/2000/svg">
...
// 충동적 소비: 복숭아 세잎 클로버
function ImpulsiveClover({ selected }: { selected?: boolean }) {
  return (
    <svg viewBox="494 445 348 160" className="w-full h-full" style={shapeFilterStyle(selected)} fill="none" xmlns="http://www.w3.org/2000/svg">
...
// 보상심리: 하늘색 십자 다이아몬드
function RewardCross({ selected }: { selected?: boolean }) {
  return (
    <svg viewBox="2044 414 238 236" className="w-full h-full" style={shapeFilterStyle(selected)} fill="none" xmlns="http://www.w3.org/2000/svg">
...
// 잘 모르겠어요: 회색 오각형
function UnsurePentagon({ selected }: { selected?: boolean }) {
  return (
    <svg viewBox="2362 452 210 204" className="w-full h-full" style={shapeFilterStyle(selected)} fill="none" xmlns="http://www.w3.org/2000/svg">
...
interface KW {
  ...
  Shape: React.FC<{ selected?: boolean }>
}
...
const KEYWORDS: KW[] = [
  ...
  { label: '합리적 소비',   w: 166, h: 166, top: '34%',   left: '8%',    Shape: RationalDiamond },
      ...
      <div className="flex-1 relative overflow-hidden min-h-0">
        {KEYWORDS.map(({ label, w, h, top, bottom, left, right, Shape }) => {
          ...
          return (
            ...
            >
              {/* SVG 아이콘: 버튼 전체를 채움 (그림자는 아이콘 모양을 따라가도록 SVG 자체에 적용) */}

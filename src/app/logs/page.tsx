...
export default function LogsPage() {
  ...
  // 월별 캘린더 데이터 fetch
  useEffect(() => {
    apiFetch<{ days: CalendarDay[] }>(`/api/records/calendar?year=${year}&month=${month}`)
      .then(res => {
        ...
        for (const day of res.data.days) {
          if (day.count > 0) {
            anyRecord = true
            const keyword = day.dominant_emotion ? (API_TO_KEYWORD[day.dominant_emotion] ?? null) : null
            if (keyword && DOT_COLORS[keyword]) {
              map[day.date] = DOT_COLORS[keyword]
            }
          }
        }
        setDotMap(map)
        setHasAnyRecord(anyRecord)
      })
      .catch(() => {})
  }, [year, month])

  // 날짜 탭 시 해당 날짜 기록 fetch
  const handleDateTap = useCallback(async (dateStr: string) => {
    setSelectedDate(dateStr)
    setModalLoading(true)
    setModalRecords([])
    try {
      const res = await apiFetch<{ items: TodayItem[] }>(`/api/home/consumptions/today?date=${dateStr}`)
      const records: SpendingRecord[] = res.data.items.map(item => ({
        id: item.consumption_id,
        title: item.title,
        category: API_TO_CATEGORY[item.category] ?? item.category,
        date: dateStr,
        photo: item.thumbnail_url,
        keyword: API_TO_KEYWORD[item.emotion_tag] ?? null,
        amount: item.amount,
        reviewDone: false,
      }))
  ...
  // 회고 대기: 백엔드에 reviewDone 개념 없음 → 섹션 표시 안 함
  ...
  return (
    <div className="relative flex flex-col max-w-md mx-auto bg-white" style={{ height: '100dvh' }}>
        ...
        <div className="grid grid-cols-7 px-3 gap-y-1">
          {cells.map((day, i) => {
            ...
            return (
              <button
                key={i}
                onClick={tappable ? () => handleDateTap(dateStr) : undefined}
      ...
      {selectedDate && (
        <DayModal
          date={selectedDate}
          records={modalLoading ? [] : modalRecords}
